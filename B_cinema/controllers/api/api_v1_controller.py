...
def api_criar_sessao():
    ...
    db.session.commit()
    return jsonify({"id": sessao.id, "mensagem": "Sessão criada"}), 201 #QUAL É O STATUS CODE PARA UMA SESSÃO CRIADA COM SUCESSO? é 201 created
...
def api_buscar_tmdb():
    termo = request.args.get("q", "") # #TODO responda por que estamos utilizando o request.args.get? para obter parâmetros de consulta da URL
